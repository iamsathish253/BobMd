...
function createArticleDC(executionContext) {
	try {
		...
		async function createDcRecords(executionContext) {
			...
			var ownerid = formContext.getAttribute("ownerid").getValue()[0].id.slice(1, -1);
			var ownerType = owner[0].entityType;
			var ownerSetName = (ownerType === "systemuser") ? "systemusers" : "teams";
					...
					.then(async function success(results) {
						...
						if (results.entities.length === 0) {
							...
							await Promise.all(dcValues.map(async (dcValue) => {
								...
								record["ownerid@odata.bind"] = `/${ownerSetName}(${ownerid})`;
		...
		if (formContext.ui.getFormType() == 1) {
			...
			await Xrm.WebApi.retrieveRecord(entityName, entityId, "?$select=bdf_previousnotescomments").then(
				async function success(existingRecord) {
                    ...
                    record.bdf_vendorcommentedon=null; // Add by Sathish to Make bdf_vendorcommentedon null on 04/02/2025
					//record.bdf_notes = '';

					var res = await Xrm.WebApi.updateRecord(entityName, entityId, record, formContext.getAttribute("bdf_notes").setValue(null));
					// if (res) {
