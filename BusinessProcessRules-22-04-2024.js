...
function setupTime(executionContext) {
    ...
    async function AssignSetupTime() {
        try {
            ...
            // First API call to retrieve records based on productTypeGuid, productSubTypeGuid, and sizeGuid
            let firstApiResult = await Xrm.WebApi.retrieveMultipleRecords("bdf_sapsetuptime", `?$filter=_bdf_producttype_value eq ${productTypeGuid} and _bdf_productsubtype_value eq ${productSubTypeGuid} and _bdf_size_value eq ${sizeGuid}&$select=bdf_setuptimeminutes`);
    
            if (firstApiResult.entities.length > 0) {
                // If results found, get the highest setup time
                for (var i = 0; i < firstApiResult.entities.length; i++) {
                    var sapSetupTimeResult = firstApiResult.entities[i];
                    if (sapSetupTimeResult.bdf_setuptimeminutes > highestSetupTime) {
                        highestSetupTime = sapSetupTimeResult.bdf_setuptimeminutes;
                    }
                }
            } else {
                // If no results, make another API call with only productTypeGuid
                let secondApiResult = await Xrm.WebApi.retrieveMultipleRecords("bdf_sapsetuptime", `?$filter=_bdf_producttype_value eq ${productTypeGuid}&$select=bdf_setuptimeminutes`);
    
                if (secondApiResult.entities.length > 0) {
                    // Get the highest setup time for the second API call
                    for (var i = 0; i < secondApiResult.entities.length; i++) {
                        var sapSetupTimeResult = secondApiResult.entities[i];
                        if (sapSetupTimeResult.bdf_setuptimeminutes > highestSetupTime) {
                            highestSetupTime = sapSetupTimeResult.bdf_setuptimeminutes;
                        }
                    }
            ...
            // If we found a valid setup time, update the record
            if (highestSetupTime !== -1) {
                var variantEntityId = formContext.data.entity.getId().slice(1, -1); // Get GUID
                var updateData = {
                    "bdf_setuptimeminutes": highestSetupTime
                };
    
                await Xrm.WebApi.updateRecord("cr60a_stg_article_master", variantEntityId, updateData);
