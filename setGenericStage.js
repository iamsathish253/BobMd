...
async function checkVANArticleGroup1(genericID) {
	...
	var error = false;
	var data = await Xrm.WebApi.retrieveMultipleRecords("cr60a_stg_article_master", "?$filter=bdf_Generic/bdf_genericid eq " + genericID + " and cr60a_vendorarticlenumber eq null and bdf_articletype eq 1");
	...
	var data = await Xrm.WebApi.retrieveMultipleRecords("cr60a_stg_article_master", "?$filter=bdf_Generic/bdf_genericid eq " + genericID + " and _bdf_articlegroup_value eq null");
	if (data.entities.length > 0) {
		Xrm.Navigation.openAlertDialog({
...
async function checkVariant1(genericID, formContext) {
	...
	var error = false;
	var data = await Xrm.WebApi.retrieveMultipleRecords("cr60a_stg_article_master", "?$orderby=bdf_outofpackagingvolume&$expand=bdf_Project($select=bdf_minorcodelookup),cr60a_ProductType&$filter=cr60a_cmstatus eq null and bdf_Generic/bdf_genericid eq " + genericID);
	for (variant of data.entities) {
...
async function fieldMandatory1() {
	try {
		...
		await Xrm.WebApi.retrieveMultipleRecords("cr60a_stg_article_master", `?$select=cr60a_articleid,_bdf_generic_value,cr60a_salestext&$filter=_bdf_generic_value eq ${Xrm.Page.data.entity.getId().slice(1, -1)}`).then(
			function success(results) {
				console.log(results);
...
function addSoftWarning3(executionContext) {
	...
	if (genericStage != null) {
		...
		if (genericStage != '1') {
			let genericID = formContext.data.entity.getId().slice(1, -1);
			Xrm.WebApi.retrieveMultipleRecords("cr60a_stg_article_master", "?$orderby=bdf_outofpackagingvolume&$expand=bdf_Project($select=bdf_minorcodelookup),cr60a_ProductType&$filter=cr60a_cmstatus eq null and bdf_Generic/bdf_genericid eq " + genericID).then(
				function success(data) {
					let message = '';
					let error = false;
