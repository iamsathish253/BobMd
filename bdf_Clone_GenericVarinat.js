	async function cloneGenericVariant(GUIDS, control) {
		debugger;
	try{
		Xrm.Utility.showProgressIndicator("Cloning Generic and Variants. Don’t refresh the page.. ");
		...
		for (GUID of GUIDS) {
			await Xrm.WebApi.retrieveRecord("bdf_generic", GUID, "?$expand=bdf_variant_generic").then(
				...
				record.bdf_flowname = "Clone Generic & Variant "; // Text
		...
		function saveGeneric(source) {
			try {
				...
				var input =
				{
					...
					//"bdf_specificcolorlkp@odata.bind": (source["_bdf_specificcolorlkp_value"] == null ? null : "/bdf_specificcolorgroups(" + source["_bdf_specificcolorlkp_value"] + ")"), //Added by Sathish 12/9/2024
					//"bdf_SpecificFinishLKP@odata.bind": (source["_bdf_specificfinishlkp_value"] == null ? null : "/bdf_specificfinishs(" + source["_bdf_specificfinishlkp_value"] + ")"), //Added by Sathish 12/9/2024
					"bdf_specificmaterial": source["bdf_specificmaterial"],

					//"bdf_ColorGroup@odata.bind": (source["_bdf_colorgroup_value"] == null ? null : "/cr60a_colorgroups("+source["_bdf_colorgroup_value"]+")"), // UnCommented by Sathish 12/9/2024
					//"bdf_FinishGroup@odata.bind": (source["_bdf_finishgroup_value"] == null ? null : "/cr60a_finishgroups("+source["_bdf_finishgroup_value"]+")"), // UnCommented by Sathish 12/9/2024
		...
		async function saveVariant(source, genericGUID, projectGUID) {
			try {
				var input =
				{
					...
					"bdf_Project@odata.bind": projectGUID == null ? null : "/bdf_projects(" + projectGUID + ")",
					"bdf_Generic@odata.bind":genericGUID == null ? null : "/bdf_generics(" + genericGUID + ")",
					...
					//"cr60a_ColorGroup@odata.bind": (source["_cr60a_colorgroup_value"] == null ? null : "/cr60a_colorgroups("+source["_cr60a_colorgroup_value"]+")"), //UnCommented by Sathish 12/9/2024
					//"cr60a_FinishGroup@odata.bind": (source["_cr60a_finishgroup_value"] == null ? null : "/cr60a_finishgroups("+source["_cr60a_finishgroup_value"]+")"), //UnCommented by Sathish 12/9/2024
					...
					//"bdf_SpecificColor@odata.bind": (source["_bdf_specificcolor_value"] == null ? null : "/bdf_specificcolorgroups(" + source["_bdf_specificcolor_value"] + ")"), //Added by Sathish 12/9/2024
					//"bdf_SpecificFinish@odata.bind": (source["_bdf_specificfinish_value"] == null ? null : "/bdf_specificfinishs(" + source["_bdf_specificfinish_value"] + ")"), //Added by Sathish 12/9/2024
					...
					// "cr60a_seatcushionthickness": ((source['cr60a_seatcushionthickness'] ==0 ||source['cr60a_seatcushionthickness'] ==null) ? null:source['cr60a_seatcushionthickness']),
					...
					"bdf_outofpackagingvolume": source['bdf_outofpackagingvolume'] < 0.001 ? 0.001 : source['bdf_outofpackagingvolume'],
				...
				async function success(result) {
						...
						updateArticleID(articleGUID); //Commented by Sathish 12/9/2024
						...
						await Xrm.WebApi.retrieveRecord("cr60a_stg_article_master", articleGUID).then(
							function success(data) {
								if (data['bdf_articletype'] != '1') {
									saveBOM(data['cr60a_stg_article_masterid'], source["cr60a_stg_article_masterid"], source);
								}
							}
						);
			...
			await Xrm.WebApi.retrieveMultipleRecords("bdf_articlebillofmaterial", "?$filter=_bdf_packagearticle_value eq " + PackageId).then(
				function success(data) {
					...
					data.entities.forEach(async function (row) {
						...
						try {
							if (idMap[row._bdf_componentarticle_value] != null && !skip) {
								var input = {
	...
	async function cloneVariant(GUIDS, control) {
		...
		if (GUIDS.length > 1) {
			...
			Xrm.Utility.showProgressIndicator("Cloning Variants. Don’t refresh the page..");
			try {
				...
				record.bdf_processstatus = 2;
				record.bdf_startdatetime = estTimeStart; // Date Time
				record.bdf_flowname = "Clone Variant"; // Text
			...
			try {
				...
				await Xrm.WebApi.createRecord("cr60a_stg_article_master", input).then(
					async function success(result) {
						...
						//Updating Article Id..................... by Sathish
						updateArticleID(articleGUID);
						// // Creating BOM................... Added by Sathish 11/21/2024
						// await Xrm.WebApi.retrieveRecord("cr60a_stg_article_master", articleGUID).then(
						// 	function success(data) {
						// 		if (data['bdf_articletype'] != '1') {
						...
						await Xrm.WebApi.retrieveMultipleRecords("bdf_articledc", `?$filter=_bdf_article_value eq '${articleGUID}'`).then(
							async function success(results) {
								...
								if (results.entities.length == 0) {
									...
									var dcValues = ["3000", "3010", "3200", "3210", "3220","3240"];
									...
									for (let i = 0; i < dcValues.length; i++) {
										...
										record["bdf_grprocessingtime"] = (i < 6) ? 6 : null/* Set your default value for other records */;
						...
						idMap[source["cr60a_stg_article_masterid"]] = articleGUID;
						saveArticlePackageMM(projectGUID, articleGUID);
						//updateArticleID(articleGUID); //Comnted by Sathish

						//Ened here by Sathish
					},
					function (error) {
						Xrm.Utility.alertDialog(error.message);
					}
...
//Commented by Sathish 11/22/2024
		async function updateArticleID(articleID) {
					...
					function success(data) {
						...
						Xrm.WebApi.updateRecord("cr60a_stg_article_master", articleID, input).then(
							function success(result) {
								productGUID = result.id;
								//Xrm.Utility.closeProgressIndicator();
								//control.refresh(); by Sathish 11/22/2024
							},
							function (error) {
								Xrm.Utility.alertDialog(error.message);
							}
						...
						if (results.entities.length > 0) {
							...
							for (let i = 0; i < results.entities.length; i++) {
								...
								if (dccodeValue !== "3220") {
									...
									entity.bdf_grprocessingtime = (i < 6) ? 999 : null/* Set your default value for other records */;
